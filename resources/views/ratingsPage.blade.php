    ...
    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 col-md-offset-2">
      <div class="panel panel-default">
        ...
        <div class="panel-body">
          {{-- if $pharmacyRatings variable is not empty --}} @if(!empty($pharmacyRatings)) {{-- setting value of i for using in form
          --}} {{-- this variable will be used in form input id to get unique input id for each form set --}}
          <?php $i = '0'; ?> @foreach ($pharmacyRatings as $pharmacyRating)
              ...
              <div class="stars">
                <form action="ratePharmacy/{{$pharmacyRating->id}}" method="post">
                  ...
                  <fieldset class="rating">
                    <input onChange="this.form.submit();" type="radio" id="star5-{{$i}}" name="rating" value="5" />
                    <label class="full" for="star5-{{$i}}" title="Awesome - 5 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star4half-{{$i}}" name="rating" value="4.5" />
                    <label class="half" for="star4half-{{$i}}" title="Pretty good - 4.5 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star4-{{$i}}" name="rating" value="4" />
                    <label class="full" for="star4-{{$i}}" title="Pretty good - 4 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star3half-{{$i}}" name="rating" value="3.5" />
                    <label class="half" for="star3half-{{$i}}" title="Meh - 3.5 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star3-{{$i}}" name="rating" value="3" />
                    <label class="full" for="star3-{{$i}}" title="Meh - 3 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star2half-{{$i}}" name="rating" value="2.5" />
                    <label class="half" for="star2half-{{$i}}" title="Kinda bad - 2.5 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star2-{{$i}}" name="rating" value="2" />
                    <label class="full" for="star2-{{$i}}" title="Kinda bad - 2 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star1half-{{$i}}" name="rating" value="1.5" />
                    <label class="half" for="star1half-{{$i}}" title="Meh - 1.5 stars"></label>
                    <input onChange="this.form.submit();" type="radio" id="star1-{{$i}}" name="rating" value="1" />
                    <label class="full" for="star1-{{$i}}" title="Sucks big time - 1 star"></label>
                    <input onChange="this.form.submit();" type="radio" id="starhalf-{{$i}}" name="rating" value="0.5" />
          ...
          <?php $i++; ?> @endforeach {{-- if $pharmacyRatings variable is empty --}} @else no pharmacies yet @endif
        </div>
      </div>
    </div>
  </div>
